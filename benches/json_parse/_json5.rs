...
pub fn baseline(b: &mut Bencher) {
  b.iter(|| {
    let parsed: serde_json::Value = json5::from_str(super::BASELINE).unwrap();
...
pub fn serial(b: &mut Bencher) {
  b.iter(|| {
    let parsed: serde_json::Value = json5::from_str(super::SERIAL).unwrap();
    assert_eq!(parsed["test100"], 100.0);
  });
}

pub fn nested(b: &mut Bencher) {
  b.iter(|| {
    let parsed: serde_json::Value = json5::from_str(super::NESTED).unwrap();
